...
# Render CycloneDDS peers (lidar Pi + VIO Pi + laptop).
...
# Render OpenVINS Kalibr-style configs so topics match the driver / bag.
...
if [[ -f "${OV_ESTIMATOR_TEMPLATE}" ]]; then
  # Estimator YAML references sibling kalibr_*.yaml basenames in the same dir.
